...
Get-ChildItem -Path $PSScriptRoot -File -Recurse -exclude *.ps1 | % {
    ...
    if ($contents -match "PersonApi") {
        $contents -replace 'PersonApi', $apiName | Set-Content $_.PSPath
        Write-Host $("'{0}': contents changed." -f $fileName)
    }

    if ($fileName -match 'PersonApi') {
        $newName = $_.Name -replace 'PersonApi', $apiName
...
Where-Object { $_.Name -match 'PersonApi' } | % {
    Write-Host $("Editing directory: '{0}'." -f $_.FullName)
    $newDirName = $_.Name -replace 'PersonApi', $apiName
